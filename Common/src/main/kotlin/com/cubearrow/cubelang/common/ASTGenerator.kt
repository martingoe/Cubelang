...
class ASTGenerator(private val outputDir: String, syntaxGrammarFile: String) {
    private var syntaxGrammar: String = File(syntaxGrammarFile).readText()
    ...
    private fun generateExpressionClass() {
        ...
        File(outputDir + "Expression.kt").writeText(expressionFileContent)
    }

    private fun generateBaseExpressionClass(): String {
        return """
                package cubelang.parser
                
                
                import cubelang.utils.Type
                import cubelang.lexer.Token
