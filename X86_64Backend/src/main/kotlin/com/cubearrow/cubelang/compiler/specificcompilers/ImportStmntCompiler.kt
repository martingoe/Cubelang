...
class ImportStmntCompiler(val context: CompilerContext) : SpecificCompiler<Expression.ImportStmnt> {
    override fun accept(expression: Expression.ImportStmnt): String {
        ...
        return "%include \"${if (expression.identifier.tokenType == TokenType.IDENTIFIER) File(File(Compiler.LIBRARY_PATH).absolutePath + "/" + expression.identifier.substring).absolutePath else File(expression.identifier.substring.substringBeforeLast(".")).absolutePath}.asm\""
    }
}
